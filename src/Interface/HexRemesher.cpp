...
HexRemesher::RetCode HexRemesher::extractHexMesh(HexMeshProps& hexMeshProps) const
{
...
HexRemesher::RetCode HexRemesher::extractPolyHexMesh(PolyMeshProps& hexMeshProps, int subdiv) const
{
...
HexRemesher::RetCode HexRemesher::extractMCMesh(PolyMeshProps& polyMeshProps) const
{
    map<VH, VH> v2v;
    map<EH, EH> e2e;
    map<FH, FH> f2f;
    ...
    polyMeshProps.allocate<IS_SINGULAR>(0);
            ...
            if (e2e.count(e) == 0)
            {
                ...
                if (a.is_valid())
                {
                    ...
                    if (mcMeshProps().isAllocated<IS_SINGULAR>() && mcMeshProps().get<IS_SINGULAR>(a))
                        polyMeshProps.set<IS_SINGULAR>(eNew, mcMeshProps().get<IS_SINGULAR>(a));
...
HexRemesher::RetCode HexRemesher::smooth(HexMeshProps& hexMeshProps, int iter) const
{
    ...
    for (int i = 0; i < iter; i++)
    {
        for (VH v : vs)
        {
            bool isOnFeatureFacet = hexMesh.is_boundary(v);
            bool onFeature = hexMeshProps.get<IS_FEATURE_V>(v);

            if (!onFeature && !isOnFeatureFacet && hexMeshProps.isAllocated<IS_FEATURE_F>())
                for (FH f : hexMesh.vertex_faces(v))
                    if (hexMeshProps.get<IS_FEATURE_F>(f))
                        isOnFeatureFacet = true;
            if (!onFeature && hexMeshProps.isAllocated<IS_FEATURE_E>())
                for (EH e : hexMesh.vertex_edges(v))
                    if (hexMeshProps.get<IS_FEATURE_E>(e))
                        onFeature = true;
            if (!onFeature && hexMeshProps.isAllocated<IS_SINGULAR>())
                for (EH e : hexMesh.vertex_edges(v))
...

            {
                ...
                for (HEH he : hexMesh.outgoing_halfedges(v))
                {
                    ...
                    // For each neighbour
                    if (!isOnFeatureFacet || hexMesh.is_boundary(hexMesh.edge_handle(he))
                        || (hexMeshProps.isAllocated<IS_FEATURE_F>()
                            && containsMatching(hexMesh.halfedge_faces(he),
                                                [&, this](const FH& f) { return hexMeshProps.get<IS_FEATURE_F>(f); })))
                    {
                ...
                {
                    set<HFH> hfs;
                    list<CH> cQ({*hexMesh.vc_iter(v)});
                    set<CH> cs({cQ.front()});
                    while (!cQ.empty())
                    {
                        CH c = cQ.front();
                        cQ.pop_front();
                        for (HFH hf : hexMesh.cell_halffaces(c))
                        {
                            if (!contains(hexMesh.halfface_vertices(hf), v))
                                continue;
                            FH f = hexMesh.face_handle(hf);
                            if (hexMesh.is_boundary(f)
                                || (hexMeshProps.isAllocated<IS_FEATURE_F>() && hexMeshProps.get<IS_FEATURE_F>(f)))
                                hfs.insert(hexMesh.opposite_halfface_handle(hf));
                            else
                            {
                                CH cNext = hexMesh.incident_cell(hexMesh.opposite_halfface_handle(hf));
                                if (!cs.count(cNext))
                                {
                                    cQ.push_back(cNext);
                                    cs.insert(cNext);
                    ...
                    for (HFH hf : hfs)
                    {
                        ...
                        Vec3d normal = (vertices[1] - vertices[0]) % (vertices[3] - vertices[0]);
                        normal.normalize();
                        if ((avgNormal | normal) < 0.9 && (avgNormal | -normal) < 0.99)
                        {
                            tooSpread = true;
                            break;
                        }
...
HexRemesher::RetCode HexRemesher::writeHexMesh(const HexMeshProps& hexMeshProps, const std::string& filename) const
{
    ...
    DLOG(INFO) << "Writing extracted hex mesh to " << filename;
...
class HaltonColors
{
  public:
    ...
    static OVM::Vec3f HSL2RGB(double h, double sl, double l)
    {
        double v;
        double r, g, b;
        r = l;
...
void HexRemesher::viewHexMesh(HexMeshProps& hexMeshProps) const
{
#ifdef MC3D_WITH_VIEWER
    ...
    vector<VCylinder> vmeshArcs;
    int vmeshPatches = 0;
    float vmeshLength = 1.0;
    float vmeshNodeScale = 1.0;
    float vmeshArcScale = 1.0;
    vector<OVM::Vec4f> vmeshMarkColors;
    OVM::Vec4f vmeshNodeColor;
    OVM::Vec4f vmeshArcColor;
    OVM::Vec4f vmeshPatchColor;
    OVM::Vec4f vmeshStandardColor;
    OVM::Vec4f vmeshFeatureColor;
    OVM::Vec4f vmeshSingularColorPos;
    OVM::Vec4f vmeshSingularColorNeg;
    float vmeshBoundaryOpacity;
    bool vmeshDrawNormalElements;
    ...
    auto computeBC = [&floodFillSheet, &floodFillLink](OpenVolumeMesh::GeometryKernel<OpenVolumeMesh::Vec3d>& ovm)
    {
        ...
        DLOG(INFO) << "Number of links: " << manyLinkEdges.size();
        ...
        DLOG(INFO) << "BC has " << blocks.size() << " blocks, " << patches.size() << " patches, " << arcs.size()
    ...
    auto vmesh = load(&hexMeshProps.mesh());
    vmeshMarkColors = vector<OVM::Vec4f>(20, OVM::Vec4f(0, 0, 0, 1.0));
        ...
        [&]()
        {
            ...
            if (mesh.is_valid())
            {
                ...
                if (ImGui::Button("Compute sizes"))
                {
                    ...
                    auto singularProp = ovm.request_property<IS_SINGULAR::value_t, IS_SINGULAR::entity_t>(
                        IS_SINGULAR::name() + "0");
                    ...
                    vmeshLength = percentileEdgeLength;
                    vmeshNodeScale = 0.3;
                    vmeshArcScale = 0.15;
                    vmeshNodeColor = Vec4d(0, 0, 0, 1.0);
                    vmeshArcColor = Vec4d(0, 0, 0, 1.0);
                    vmeshPatchColor = Vec4d(164 / 255.0, 141 / 255.0, 252 / 255.0, 1.0);
                    vmeshStandardColor = Vec4d(164 / 255.0, 141 / 255.0, 252 / 255.0, 1.0);
                    vmeshFeatureColor = Vec4d(0, 0, 0.55, 1.0);
                    vmeshSingularColorPos = Vec4d(0.00, 0.00, 0.70, 1.0);
                    vmeshSingularColorNeg = Vec4d(0.70, 0.00, 0.00, 1.0);
                    vmeshBoundaryOpacity = 3.0;
                    vmeshDrawNormalElements = true;
                        ...
                        while (!cellQ.empty())
                        {
                            ...
                            for (HFH hf : ovm.cell_halffaces(cell))
                            {
                                if (features[ovm.face_handle(hf)] == 0
                                    && (ovm.face_handle(hf).idx() % 3 != 0
                                        || ovm.face_handle(hf).idx() % 2 == 0))
                                {
                ...
                {
                    auto& show = vmeshDrawNormalElements;
                    show = !show;
                    toggle = true;
                }
                    ...
                    if (nodesVis.empty() || toggle)
                    {
                        ...
                        for (int i = 0; i < 5; i++)
                        {
                            if (ovm.property_exists<MC_NODE_ID::value_t, MC_NODE_ID::entity_t>(MC_NODE_ID::name()
                                                                                                   + std::to_string(i)))
                                num = i;
                            if (ovm.property_exists<IS_FEATURE_V::value_t, IS_FEATURE_V::entity_t>(
                                    IS_FEATURE_V::name() + std::to_string(i)))
                                num2 = i;
                            if (ovm.property_exists<MARK_N::value_t, MARK_N::entity_t>(MARK_N::name()
                                                                                           + std::to_string(i)))
                                num3 = i;
                            if (ovm.property_exists<IS_SINGULAR::value_t, IS_SINGULAR::entity_t>(
                                    IS_SINGULAR::name() + std::to_string(i)))
                                num4 = i;
                        }
                        ...
                        auto markProp = ovm.request_property<MARK_N::value_t, MARK_N::entity_t>(
                            MARK_N::name() + std::to_string(num3));
                        ...
                        for (VH v : ovm.vertices())
                            ...
                            )
                            {
                                if (visMarkings)
                                {
                                    ...
                                    sphere.set_color(vmeshMarkColors[markProp[v]]);
                                    sphere.set_scale((float)(vmeshLength * vmeshNodeScale));
                                }
                                else
                                {
                                    int nESingular = 0;
                                    ...
                                    auto sphere = nodesVis.back();
                                    sphere.set_position(pos);
                                    sphere.set_color(!sing ? vmeshNodeColor
                                                           : ((nNeg == 0 || (nPos % 2) == 1) > 0
                                                                  ? vmeshSingularColorPos
                                                                  : vmeshSingularColorNeg));
                                    sphere.set_scale((sing || featureVprop[v] ? 3.0f : 1.0f)
                                                     * (float)(vmeshLength * vmeshNodeScale));
                                }
                            }
                    }
                    else
                ...
                if (ImGui::DragFloat("Node scale", &vmeshNodeScale, 0.05f, 0.05f, 10.0f, "%.2f") && mesh.is_valid())
                {
                    auto& nodesVis = vmeshNodes;
                    for (auto sphere : nodesVis)
                        sphere.set_scale((float)(vmeshLength * vmeshNodeScale));
                    ...
                    if (arcVis.empty() || toggle)
                    {
                        ...
                        for (int i = 0; i < 5; i++)
                        {
                            if (ovm.property_exists<MC_ARC_ID::value_t, MC_ARC_ID::entity_t>(MC_ARC_ID::name()
                                                                                                 + std::to_string(i)))
                                num = i;
                            if (ovm.property_exists<IS_FEATURE_E::value_t, IS_FEATURE_E::entity_t>(
                                    IS_FEATURE_E::name() + std::to_string(i)))
                                num2 = i;
                        ...
                        auto markProp = ovm.request_property<MARK_A::value_t, MARK_A::entity_t>(
                            MARK_A::name() + std::to_string(num3));
                        auto isSingularProp = ovm.request_property<IS_SINGULAR::value_t, IS_SINGULAR::entity_t>(
                            IS_SINGULAR::name() + std::to_string(num4));
                        ...
                        for (EH e : ovm.edges())
                            ...
                            )
                            {
                                if (visMarkings)
                                {
                                    ...
                                    float r = (float)(vmeshLength * vmeshArcScale);
                                    cylinder.set_scale(r, (float)dir.length(), r);
                                    cylinder.set_color(vmeshMarkColors[markProp[e]]);
                                ...
                                else
                                {
                                    ...
                                    float r = (isSingularProp[e] || featureEprop[e] ? 3.0f : 1.0f)
                                              * (float)(vmeshLength * vmeshArcScale);
                                    cylinder.set_scale(r, (float)dir.length(), r);
                                    cylinder.set_color(
                                        ((isSingularProp[e] || featureEprop[e])
                                             ? (isSingularProp[e] > 0 ? vmeshSingularColorPos : vmeshSingularColorNeg)
                                             : vmeshArcColor));
                                    cylinder.set_direction(dir);
                                }
                            }
                ...
                if (ImGui::DragFloat("Arc scale", &vmeshArcScale, 0.05f, 0.05f, 10.0f, "%.2f") && mesh.is_valid())
                {
                    auto& arcVis = vmeshArcs;
                    for (auto cylinder : arcVis)
                    {
                        auto scale = cylinder.get_scale<glm::vec3>();
                        auto r = (float)(vmeshLength * vmeshArcScale);
                        cylinder.set_scale(r, scale.y, r);
                    }
                }
                if (ImGui::ColorEdit4(
                        "Arc color", vmeshArcColor.data(), ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel))
                {
                    auto& arcVis = vmeshArcs;
                    auto color = vmeshArcColor;
                    if (!visMarkings)
                        for (auto cylinder : arcVis)
                            cylinder.set_color(color);
                }
                if (ImGui::Button("COLOR EVERYTHING TEST"))
                {
                    auto& ovm = *mesh.get_ovm();
                    for (HFH hf : ovm.halffaces())
                        mesh.set_color(hf, Vec4f(1.0, 0.0, 0.0, 1.0));
                    // for (FH f : ovm.faces())
                    //     mesh.set_color(f, Vec4f(1.0, 0.0, 0.0, 1.0));
                    // for (CH c : ovm.cells())
                    //     mesh.set_color(c, Vec4f(1.0, 0.0, 0.0, 1.0));
                }
                if (ImGui::Button("Toggle Patch Viz") || toggle)
                {
                    auto& patchVis = vmeshPatches;
                    auto& ovm = *mesh.get_ovm();
                    if (!toggle)
                        patchVis = patchVis == 5 ? 0 : patchVis + 1;
                    auto patchColor = vmeshPatchColor;
                    auto restColor = vmeshStandardColor;
                    int num = 0;
                    int num2 = 0;
                    int num3 = 0;
                    int num4 = 0;
                    for (int i = 0; i < 5; i++)
                    {
                        ...
                        if (ovm.property_exists<IS_FEATURE_F::value_t, IS_FEATURE_F::entity_t>(IS_FEATURE_F::name()
                                                                                                   + std::to_string(i)))
                            num2 = i;
                        if (ovm.property_exists<MARK_P::value_t, MARK_P::entity_t>(MARK_P::name()
                                                                                       + std::to_string(i)))
                            num3 = i;
                        if (ovm.property_exists<MARK_B::value_t, MARK_B::entity_t>(MARK_B::name()
                                                                                       + std::to_string(i)))
                            num4 = i;
                    }
                    if (visMarkings)
                    {
                        if (patchVis % 2 == 0)
                        {
                            ...
                            for (HFH hf : ovm.halffaces())
                            {
                                auto color = vmeshMarkColors[markPropP[ovm.face_handle(hf)]];

                                bool isBoundary = ovm.is_boundary(ovm.face_handle(hf));
                                if (isBoundary)
                                    color[3] *= vmeshBoundaryOpacity;
                        ...
                        else
                        {
                            ...
                            for (CH c : ovm.cells())
                            {
                                mesh.set_color(c, vmeshMarkColors[markPropB[c]]);
                            }
                            ...
                            for (HFH hf : ovm.halffaces())
                            {
                                ...
                                auto color = vmeshMarkColors[markPropB[ovm.incident_cell(hfInc)]];
                    ...
                    else
                    {
                        ...
                        else if (patchVis == 1)
                        {
                            ...
                            for (HFH hf : ovm.halffaces())
                            {
                                auto color = patchProp[ovm.face_handle(hf)] == -1
                                                 ? restColor
                        ...
                        else if (patchVis == 3)
                        {
                            ...
                            auto blockProp = ovm.request_property<MC_BLOCK_ID::value_t, MC_BLOCK_ID::entity_t>(
                                MC_BLOCK_ID::name() + std::to_string(num8));
                            auto featureFprop = ovm.request_property<IS_FEATURE_F::value_t, IS_FEATURE_F::entity_t>(
                                IS_FEATURE_F::name() + std::to_string(num2));
                            for (HFH hf : ovm.halffaces())
                            {
                        ...
                        else if (patchVis == 4)
                        {
                            ...
                            for (HFH hf : ovm.halffaces())
                            {
                                auto color = (features[ovm.face_handle(hf)]
                                                      && !ovm.is_boundary(ovm.face_handle(hf))
                                                  ? vmeshFeatureColor
                                                  : restColor);
                                bool isBoundary = ovm.is_boundary(ovm.face_handle(hf));
                                      ...
                                      ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel))
                {
                    ...
                    if (patchVis == 1)
                    {
                        ...
                        int num = 0;
                        for (int i = 0; i < 5; i++)
                            if (ovm.property_exists<MC_PATCH_ID::value_t, MC_PATCH_ID::entity_t>(
                                    MC_PATCH_ID::name() + std::to_string(i)))
                            {
                                num = i;
                                break;
                            }
                ...
                if (ImGui::DragFloat("Boundary opacity", &vmeshBoundaryOpacity, 0.05f, 0.0f, 100.0f, "%.2f")
                    && mesh.is_valid())
                {
                    auto& patchVis = vmeshPatches;
                    auto& ovm = *mesh.get_ovm();
